...
class RealtimeJapaneseTranscriber:
    ...
    def setup_gui(self):
        ...
        self.root = tk.Tk()
        self.root.title("リアルタイム字幕システム（キーワード付き） / Real-time Subtitle System (with keyword)")
    ...
    def create_header(self, parent):
        ...
        title_label = tk.Label(header_frame, 
                              text="リアルタイム字幕システム / Real-time Subtitle System",
        ...
        subtitle_label = tk.Label(header_frame,
                                 text="Real-time Subtitle Transcription System",
                                 font=('Segoe UI', 12),
    ...
    def create_topic_section(self, parent):
        ...
        topic_label.pack(anchor=tk.W, pady=(0, 8))
        ...
        language_label = tk.Label(topic_inner, 
                                 text="転写言語： / Transcription Language:",
                                 font=('Yu Gothic', 14, 'bold'),
                                 fg=self.colors['text'],
                                 bg=self.colors['surface'])
        language_label.pack(anchor=tk.W, pady=(0, 8))
    ...
    def create_subtitle_section(self, parent):
        ...
        subtitle_frame = ttk.LabelFrame(parent, 
                                       ...
                                       padding=20)
        subtitle_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 20))
        ...
        while self.is_recording:
            try:
                ...
                if first_request and self.meeting_topic:
                    prompt = f"以下の音声は「{self.meeting_topic}」というキーワードに関連しています。このキーワードを念頭に置いて、日本語で正確に文字起こししてください。 / The following audio is related to the keyword “{self.meeting_topic}”. Please transcribe accurately in Japanese while keeping this keyword in mind."
                    first_request = False
                else:
                ...
                if transcription:
                    ...
                    debug_text = f"応答時間: {latency}ms | プロンプト長: {len(prompt)} | バッファ長: {len(self.audio_buffer)/self.RATE:.1f}s / Response time: {latency}ms | Prompt length: {len(prompt)} | Buffer length: {len(self.audio_buffer)/self.RATE:.1f}s"
    ...
    def build_context_prompt(self):
        ...
        return f"これは日本語の音声の続きです。前の文脈：{context_text} / This is a continuation of Japanese audio. Previous context: {context_text}"
